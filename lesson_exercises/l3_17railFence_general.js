/*Problem1:
Take a string decoded and encode it for a 'railfence' 
  with arbitrary number rails

input: string, encoded message and number of rails
...
function encodeMessage(message, numberOfRails) {
  lineArrays = [];
  for (let num = 0; num < numberOfRails; num += 1){
    lineArrays.push([])
  } 
...
console.log(railFenceEncode('WE ARE DISCOVERED FLEE AT ONCE', 3)  === 'WECRLTEERDSOEEFEAOCAIVDEN');
console.log(railFenceEncode('WE AR%E-DISCOVERED_FLEE AT ONCE', 3) === 'WECRLTEERDSOEEFEAOCAIVDEN');
console.log(railFenceDecode('WECRLTEERDSOEEFEAOCAIVDEN')); // output railFence

console.log(railFenceEncode('I WANT TO EAT A BIG PIECE OF PIZZA', 3) === 'ITAIEFZWNTETBGICOPZAAOAPEI');
console.log(railFenceDecode('ITAIEFZWNTETBGICOPZAAOAPEI', 3));
